 ...
 * }
 *
 * // 4. Log or visualize the 3D pose (e.g. with AdvantageScope).
...
public class FuelBumpSim {
     ...
     */
    public Pose3d update(Pose2d robotPose2d, Vector2 fieldRelativeSpeeds, int subticks, int subTickNum) {
        ...
        for (int lineIdx = BUMP_LINE_FIRST; lineIdx <= BUMP_LINE_LAST; lineIdx++) {
            double gax = handleModuleBumpCollision(
                    onRamp ? simXPos : robotPose2d.getX(), robotPose2d.getY(), onRamp ? simXVel : vx, lineIdx);
            if (!Double.isNaN(gax)) {
                gravAccelXSum += gax;
                contactCount++;
            }
     ...
     */
    private double handleModuleBumpCollision(double worldX, double worldY, double currentXVel, int lineIdx) {
