...
public final class AutoLogLevelManager {
     ...
     * where {@code @AutoLogLevel} is used must be within the same package as {@code Robot} (or a
    ...
    private static void addObjectImpl(Object root) {
        ...
        getAllMethods(root.getClass()).forEach((methodAndDeclaringClass) -> {
            ...
            if (method.isAnnotationPresent(AutoLogLevel.class)) {
                ...
                // Get key
                String keyParameter = annotation.key();
                String key = makeKey(keyParameter, method.getName(), declaringClass, root);

                // Register method
                registerField(key, method.getReturnType(), () -> {
                    try {
        ...
        getAllFields(root.getClass()).forEach((fieldAndDeclaringClass) -> {
            ...
            if (field.isAnnotationPresent(AutoLogLevel.class)) {
                ...
                // Register field
                registerField(key, field.getType(), () -> {
                    try {
